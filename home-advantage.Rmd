...
operationalisations$study <- gsub("(.*).rxs.Rmd.*", "\\1", operationalisations$study);
...
## The full initial list of extracted operationalisations {.tabset}
...
rxs_extractedListEntities(rxs$rxsTrees);
    ...
    if ((is.null(nd$value$assoc.r) || is.na(nd$value$assoc.r)) &&
      ...
      if (is.null(nd$value$assoc.n) || is.na(nd$value$assoc.n)) {
        if (is.null(nd$value$assoc.subsample) || is.na(nd$value$assoc.subsample)) {
          ### Use general study n
          n <- nd$root$methods$N$value;
        } else {
          ### Use n from a subsample
          n <- FindNode(node=node$root,
